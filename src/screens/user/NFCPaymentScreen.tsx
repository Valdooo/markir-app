...
export const NFCPaymentScreen = ({ navigation }: any) => {
  ...
  const processNFCPayment = async (tag: any) => {
    try {
      ...
      Alert.alert(
        '✅ Pembayaran Berhasil!',
        `Lokasi: ${location}\nBiaya: Rp ${parkingFee.toLocaleString('id-ID')}\n\nSaldo Anda telah berkurang.`,
        [{ text: 'OK', onPress: () => navigation.goBack() }]
      );

      // In real app: Update Redux balance, save to backend
    } catch (error) {
      Alert.alert('❌ Pembayaran Gagal', 'Terjadi kesalahan saat memproses pembayaran. Silakan coba lagi.');
    }
  };
      ...
      <View style={styles.content}>
        ...
        <View style={styles.balanceCard}>
          ...
          <Text style={styles.balanceAmount}>
            Rp {((user?.e_wallet_balance || 0)).toLocaleString('id-ID')}
          </Text>
        </View>
