...
const icons: { name: keyof typeof Ionicons.glyphMap }[] = [
  { name: 'home' },
  { name: 'person' },
  { name: 'radio' }, // NFC icon (radio waves)
  { name: 'wallet' },
...
export const CustomTabBar = ({ state, descriptors, navigation }: any) => {
  ...
  // Handle center NFC button with role-based navigation
  const handleNFCPress = () => {
    Vibration.vibrate(15); // Haptic feedback
    
    // Role-based navigation
    if (user?.role === 'admin') {
      // Admin: Go to NFC Payment screen for reading tags
      navigation.navigate('Home'); // Or create AdminNFCScreen
    } else {
      // User: Go to Wallet for payments
